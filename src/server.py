    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            try:
                ...
                elif url.path in ['/','/app.js','/style.css','/design.css','/grid-substation.png','/grid-engineers.png']:
                    name={'/':'index.html','/app.js':'app.js','/style.css':'style.css','/design.css':'design.css','/grid-substation.png':'grid-substation.png','/grid-engineers.png':'grid-engineers.png'}[url.path]
                    payload=(ROOT/'src/web'/name).read_bytes()
                    self.send_response(200);self.send_header('Content-Type',{'index.html':'text/html','app.js':'text/javascript','style.css':'text/css','design.css':'text/css','grid-substation.png':'image/png','grid-engineers.png':'image/png'}[name]+'; charset=utf-8');self.end_headers();self.wfile.write(payload);return
