...
function focusedTransformer(){
  const assets=[...state.assets].sort((a,b)=>a.id.localeCompare(b.id));
  const asset=assets.find(a=>a.id===selectedAssetId)||assets[0];selectedAssetId=asset.id;
  ...
  return `<section class="focus-panel ${level}" aria-label="Selected transformer live conditions"><div class="focus-head"><div><span class="focus-kicker">TRANSFORMER FOCUS / LIVE SYNTHETIC DATA</span><h2>${asset.name}</h2><p>${asset.id} · ${asset.area} area</p></div><label for="transformer-select">Choose transformer<select id="transformer-select">${assets.map(a=>`<option value="${a.id}" ${a.id===asset.id?'selected':''}>${a.id} — ${a.name}</option>`).join('')}</select></label></div><div class="focus-summary"><div class="risk-orb"><span>24h risk</span><strong>${pct(asset.risk)}</strong><em>${label}</em></div><div class="focus-impact"><span>${fmt(asset.customers)} customers</span><span>${asset.critical_sites} critical sites</span><span>${pct(asset.backup_fraction)} backup coverage</span><b>${asset.task}</b></div></div><div class="focus-readings">${readings.map(([name,value,unit])=>`<div><span>${name}</span><strong>${value}<small>${unit}</small></strong></div>`).join('')}</div><div class="diagnostics"><div class="diagnostics-head"><div><span>CONDITION DIAGNOSIS</span><h3>What needs attention</h3></div><b>${issues.length} ${issues.length===1?'issue':'issues'} detected</b></div>${issues.length?`<div class="issue-grid">${issues.map(issue=>`<article class="issue ${issue.severity}"><i aria-hidden="true">${issue.severity==='critical'?'!':'↑'}</i><div><span>${issue.severity}</span><h4>${issue.title}</h4><p><strong>${issue.value}</strong> · ${issue.guide}</p><small>${issue.action}</small></div></article>`).join('')}</div>`:'<div class="all-clear"><i>✓</i><div><strong>No screening thresholds breached</strong><p>Current synthetic readings are inside the displayed operating guides.</p></div></div>'}<p class="diagnostic-note">Rule-based screening of synthetic readings. Confirm any concern with field measurements and qualified personnel.</p></div></section>`;
}
