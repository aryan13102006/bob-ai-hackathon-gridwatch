...
function simulation(){
  const opts=state.assets.map(a=>`<option value="${a.id}">${a.name} (${a.id})</option>`).join('');
  function tintFor(r){return r>=0.65?'sim-tint-red':r>=0.35?'sim-tint-orange':'sim-tint-green'}
  function labelFor(r){return r>=0.65?{level:'CRITICAL',color:'#c0392b'}:r>=0.35?{level:'ELEVATED',color:'#c07000'}:{level:'NORMAL',color:'#2a7a40'}}
  ...
  function buildSVG(a){
    ...
    const arcPaths=Array.from({length:arcs},(_,i)=>{const x=160+i*28,dy=i%2===0?-18:18;return `<path class="sim-arc" d="M${x} 118 Q${x+12} ${118+dy} ${x+24} 118" fill="none" stroke-width="2.5" stroke-linecap="round" style="animation-delay:${i*0.22}s;animation-duration:${dur}"/>`;}).join('');
    const loadBar=Math.round((s.load_pct/140)*60),tempFill=Math.round(Math.max(0,Math.min(1,(s.temperature_c-30)/95))*80);
    return `<svg class="sim-svg ${tint}" viewBox="0 0 340 300" role="img" aria-label="Transformer simulation for ${a.name}"><defs><radialGradient id="sg-${a.id}" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="currentColor" stop-opacity="0.18"/><stop offset="100%" stop-color="currentColor" stop-opacity="0"/></radialGradient><filter id="sb-${a.id}"><feGaussianBlur stdDeviation="4"/></filter></defs><ellipse cx="170" cy="160" rx="130" ry="110" fill="url(#sg-${a.id})" filter="url(#sb-${a.id})"/><rect x="60" y="245" width="220" height="12" rx="6" fill="currentColor" opacity="0.12"/><rect class="sim-tank" x="95" y="125" width="150" height="120" rx="10"/><rect x="100" y="130" width="30" height="110" rx="6" fill="white" opacity="0.07"/><rect x="68" y="145" width="12" height="18" rx="3" fill="currentColor" opacity="0.35"/><rect x="68" y="170" width="12" height="18" rx="3" fill="currentColor" opacity="0.35"/><rect x="68" y="195" width="12" height="18" rx="3" fill="currentColor" opacity="0.35"/><rect x="260" y="145" width="12" height="18" rx="3" fill="currentColor" opacity="0.35"/><rect x="260" y="170" width="12" height="18" rx="3" fill="currentColor" opacity="0.35"/><rect x="260" y="195" width="12" height="18" rx="3" fill="currentColor" opacity="0.35"/><rect x="130" y="90" width="14" height="38" rx="5" fill="currentColor" opacity="0.55"/><rect x="163" y="85" width="14" height="43" rx="5" fill="currentColor" opacity="0.55"/><rect x="196" y="90" width="14" height="38" rx="5" fill="currentColor" opacity="0.55"/><circle cx="137" cy="87" r="7" fill="currentColor" opacity="0.7"/><circle cx="170" cy="82" r="7" fill="currentColor" opacity="0.7"/><circle cx="203" cy="87" r="7" fill="currentColor" opacity="0.7"/><line x1="137" y1="80" x2="137" y2="50" stroke="currentColor" stroke-width="2.5" opacity="0.4"/><line x1="170" y1="75" x2="170" y2="45" stroke="currentColor" stroke-width="2.5" opacity="0.4"/><line x1="203" y1="80" x2="203" y2="50" stroke="currentColor" stroke-width="2.5" opacity="0.4"/>${arcPaths}<rect x="286" y="130" width="12" height="100" rx="4" fill="currentColor" opacity="0.12"/><rect x="286" y="${230-tempFill}" width="12" height="${tempFill}" rx="4" fill="currentColor" opacity="0.55"/><text x="292" y="125" text-anchor="middle" font-size="9" fill="currentColor" opacity="0.7">°C</text><rect x="42" y="130" width="12" height="100" rx="4" fill="currentColor" opacity="0.12"/><rect x="42" y="${230-loadBar}" width="12" height="${loadBar}" rx="4" fill="currentColor" opacity="0.55"/><text x="48" y="125" text-anchor="middle" font-size="9" fill="currentColor" opacity="0.7">LD</text><circle cx="170" cy="185" r="55" fill="none" stroke="currentColor" stroke-width="1.5" opacity="0.18" class="sim-pulse" style="animation-duration:${dur}"/><circle cx="170" cy="185" r="72" fill="none" stroke="currentColor" stroke-width="1" opacity="0.1" class="sim-pulse" style="animation-duration:${dur};animation-delay:0.3s"/><text x="170" y="180" text-anchor="middle" font-size="22" font-weight="700" fill="currentColor" opacity="0.85">${pct(r)}</text><text x="170" y="198" text-anchor="middle" font-size="10" fill="currentColor" opacity="0.6">FAILURE RISK</text><text x="170" y="272" text-anchor="middle" font-size="13" font-weight="600" fill="currentColor" opacity="0.75">${a.name}</text><text x="170" y="287" text-anchor="middle" font-size="10" fill="currentColor" opacity="0.5">${a.id} · ${a.area} area</text></svg>`;
  }
  function buildView(id){
    const a=state.assets.find(x=>x.id===id)||state.assets[0],lbl=labelFor(a.risk);
    return `<div class="sim-shell"><div class="sim-header"><div><p class="eyebrow">TRANSFORMER SIMULATION / LIVE SCENARIO</p><h2>Equipment State Monitor</h2><p class="muted">Select any asset to visualise its current simulated state under the active weather scenario.</p></div><label class="sim-select-wrap" for="sim-pick">Asset<select id="sim-pick" class="sim-select">${opts}</select></label></div><div class="sim-layout"><div class="sim-visual-col"><div class="sim-visual-wrap sim-wrap-${tintFor(a.risk)}" id="sim-visual">${buildSVG(a)}</div><div class="sim-status-pill" style="--pill-color:${lbl.color}">${lbl.level} — ${pct(a.risk)} risk · threshold ${pct(state.threshold)}</div></div><div class="sim-info-col" id="sim-info">${statusBlocks(a)}</div></div><p class="muted sim-foot">All asset names, sensor readings and failure probabilities are synthetic simulation fixtures. Weather is an operator-entered scenario.</p></div>`;
  }
  setTimeout(()=>{const pick=$('sim-pick');if(pick)pick.onchange=()=>{const a=state.assets.find(x=>x.id===pick.value)||state.assets[0],lbl=labelFor(a.risk);$('sim-visual').innerHTML=buildSVG(a);$('sim-visual').className=`sim-visual-wrap sim-wrap-${tintFor(a.risk)}`;$('sim-info').innerHTML=statusBlocks(a);document.querySelector('.sim-status-pill').textContent=`${lbl.level} — ${pct(a.risk)} risk · threshold ${pct(state.threshold)}`;document.querySelector('.sim-status-pill').style.setProperty('--pill-color',lbl.color);};},0);
